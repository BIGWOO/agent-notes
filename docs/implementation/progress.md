...
| P2-004 | OpenClaw workflow dry-run | done | [`../specs/integrations.md`](../specs/integrations.md) | `integrate openclaw --dry-run` 不寫檔，未知 workflow shape 有 hints | 已新增 OpenClaw dry-run skeleton、候選 workflow/config 摘要與 recovery hints；真實 workflow shape 未確認前不 apply；init preview 只新增 dry-run next command，不 apply；驗證：`npm test -- --run tests/integrate.test.ts tests/init.test.ts`、`npm run typecheck` |
| P2-005 | Apply safety / backup / rollback tests | done | [`../specs/write-safety.md`](../specs/write-safety.md) | apply 前 backup failure、hash conflict、rollback 與 symlink 防護有 tests | 已補 Codex integration apply 的 lock conflict、planning 後 hash conflict、post-write failure rollback tests；其他 adapter apply 開放前仍需依同矩陣補 fixture tests；驗證：`npm test -- --run tests/integrate.test.ts`、`npm run typecheck` |
| P2-006 | Agent config path detection and recovery hints | done | [`../specs/integrations.md`](../specs/integrations.md) | 偵測摘要不洩漏本機絕對路徑，找不到 config 時提供下一步 | 已新增 checked config candidate basenames、safe detected summary 與 config root source hints；dry-run output 使用 basename、公開 env 名稱、短 hash 或 redacted summary；驗證：`npm test -- --run tests/integrate.test.ts tests/init.test.ts`、`npm run typecheck` |
| P2-007 | Validation / scenario matrix update | done | [`scenario-matrix.md`](scenario-matrix.md)、[`validation.md`](validation.md) | Phase 2 dry-run、unsupported apply、path detection 與 no-write cases 有情境覆蓋 | 已更新 scenario matrix integration cases 與 validation Phase 2 evidence；驗證：`npm test`、`npm run typecheck`、`npm run lint`、`npm run build`、CLI smoke |
