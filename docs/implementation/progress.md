...
| P1-003 | Vault Init | in-progress | [`../specs/init-onboarding.md`](../specs/init-onboarding.md) | fresh/already/partial/unsafe/non-interactive init cases 有測試 | 已完成非互動 `init` dry-run/apply、already-initialized idempotent rerun、existing valid vault rejection、existing non-agent dir、invalid config、target file 與 Git worktree unsafe checks；已通過本地 CLI smoke test。尚待補齊 interactive、resume/rollback、partial init 與第一個 project / integration onboarding |
