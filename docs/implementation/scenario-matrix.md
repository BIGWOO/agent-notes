...
| ID | Scenario | Command / Steps | Exit Code | Files Written | Must Not Write | Assertions | Source Spec |
| --- | --- | --- | --- | --- | --- | --- | --- |
| S-INT-001 | list 不需 Agent Notes config | `agent-notes integrate --list` before init | `OK` | none | local config、hook config | Codex 可偵測狀態可讀；Claude Code / OpenClaw dry-run-only | [`integrations.md`](../specs/integrations.md) |
...
| S-INT-004 | init integration preview | 選擇 preview integrations | `OK` | none for agent hook/config | Codex/Claude/OpenClaw config | init 只輸出 `integrate --list`、`claude-code --dry-run`、`openclaw --dry-run` preview commands；不寫 agent 設定 | [`integrations.md`](../specs/integrations.md) |
...
| S-INT-007 | Claude Code dry-run skeleton | `agent-notes integrate claude-code --dry-run` | `OK` | none | hook config、lock、backup | 顯示 hook command template、候選 config summary、checked candidate basenames 與 hints；不輸出本機絕對路徑 | [`integrations.md`](../specs/integrations.md) |
...
| S-INT-009 | OpenClaw dry-run skeleton | `agent-notes integrate openclaw --dry-run` | `OK` | none | workflow config、lock、backup | 顯示 workflow command template、候選 config summary、checked candidate basenames 與 hints；不輸出本機絕對路徑 | [`integrations.md`](../specs/integrations.md) |
| S-INT-010 | OpenClaw apply 未支援 | `agent-notes integrate openclaw --apply --yes --binary <stable>` | `INTEGRATION_UNSUPPORTED` | none | workflow config、backup | 真實 workflow shape 未 fixture-driven 驗證前不寫入 | [`integrations.md`](../specs/integrations.md) |
| S-INT-011 | Codex apply lock conflict | pre-create `integrate-codex.lock` then `integrate codex --apply --yes --binary <stable>` | `WRITE_CONFLICT` | none | Codex config、backup | 不覆蓋其他程序 lock；config 不變 | [`write-safety.md`](../specs/write-safety.md) |
| S-INT-012 | Codex apply hash conflict | Codex config 在 planning 後 apply 前被外部修改 | `WRITE_CONFLICT` | none | overwrite external config change、backup、stale lock | 保留外部變更；本次 lock 釋放 | [`write-safety.md`](../specs/write-safety.md) |
| S-INT-013 | Codex apply rollback | Codex config 寫入後發生 post-write integration failure | `INTEGRATION_APPLY_FAILED` | backup only | broken config、lock | rollback 還原原 config；backup 保留供 manual recovery | [`write-safety.md`](../specs/write-safety.md) |
...
- OpenClaw Phase 2 前段只提供 dry-run skeleton；apply 必須等真實 workflow shape、backup/rollback tests 與 review 完成。
