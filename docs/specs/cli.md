...
| `integrate openclaw --dry-run` | none | `--binary` | none | `INTEGRATION_BINARY_UNSTABLE` |
| `integrate openclaw --apply` | none | `--binary`、`--yes` | none | `INTEGRATION_UNSUPPORTED` |
...
Phase 1 只承諾 Codex integration。Phase 2 前段開始提供 Claude Code 與 OpenClaw dry-run skeleton。
...
- `integrate openclaw --dry-run` 只顯示 workflow command template、候選 config/workflow 摘要與 recovery hints，不寫檔。
- `integrate openclaw --apply` 在真實 workflow shape、backup、rollback tests 與 review 完成前回 `INTEGRATION_UNSUPPORTED`。
