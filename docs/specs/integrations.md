...
Phase 2 前段先新增共用 adapter shape 與 Claude Code dry-run skeleton。此 scope 不代表 Claude Code apply 已可用。
...
- detected config path 或找不到 config 的原因
...
| --- | --- | --- | --- |
| list without Agent Notes config | `agent-notes integrate --list` | `OK` | 不要求先 init；顯示 Codex 可偵測狀態，Claude/OpenClaw coming soon |
